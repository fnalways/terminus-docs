...
我们基于“每个 Community Application 都属于一个独立的 namespace”的基础概念，衍生了以下机制：
...
`Cluster Scoped Application`是一种特殊类型的 Community Application。这个类型的应用：

- 只有 Admin 可以安装。
- 对于每个 `Cluster Scoped Application`, 整个集群只能安装一个实例。
