...
对于 LP 而言, 有三大块配置内容, 其中 bridge 相关的配置是系统运行的基础, 只有完成此部分配置后系统才能开始工作. amm 和 monitor 分别控制对冲策略和监控报警, 我们为您预设了简单的配置方案可以直接使用, 但是我们建议您根据自身情况修改配置, 以获得更好的执行结果

### Configure A Bridge

此章节将会指引您完成 otmoic 系统中 bridge 的相关配置, bridge 在 otmoic 系统中代表一座特定的桥, 您可以简单的理解为交易所中的交易对, 但是是单向的.
bridge 的配置有两种情况

- 第一次配置时您可以在 guide 页面进行所有信息的配置
- 针对后续更多的 bridge, 您需要单独进入特定项目的配置页面进行配置
...
在这一步当中我们预设了一些 token 的配置, 方便您快速完成配置, 使用测试网络时是测试 token, 使用正式网络时是一些常见 token
...
在上面的动画中您可能发现了我们连续配置了两个 token, 因为要让一座桥正常工作至少需要两端也就是两个 token. 举个例子, 如果我们配置一座用 eth 上 usdt 交换 bsc 上 usdt 的桥, 则需要配置 eth 上 usdt 的 token 信息和 bsc 上 usdt 的 token 信息
:::
...
此步骤至少需要配置一个与 dst 一侧 token 所在链相同的 wallet, 在交换过程中, src 一侧是 LP 收到的 token, 而 dst 一侧是 LP 需要转出的 token, dst 一侧的 wallet 就是为了进行这个转出操作而配置的, 当然, 除了与 dst token 处于相同链以外, 您的 wallet 中还应该有一定量的 gas 以及 dst token 用以进行交换.
如果 src 一侧与 dst 一侧不是相同的链, 或者您希望用不同的 wallet 进行收款, 那么就需要再为 src 一侧配置一个 wallet
...
3. Configure account
   :::info account
   配置账户时您仅需要输入此 LP 的介绍信息即可, **lp_id** 将使用您的 TerminusName, 并且交换流程中的身份验证也将基于您的 TerminusName
...
侧边栏中有上述每个步骤的单独配置页面, 功能与引导流程中完全相同, 在您需要配置更多的 bridge 时, 只需要在这些信息中分别补充新 bridge 缺失的信息, 之后再进入 bridge 配置页面配置新的 bridge 即可
...
我们在 Monitoring - Create 页面中提供了编写自定义监控脚本的功能, 您可以用 ts 编写自己的监控策略
...
- 编码
  在第一步中输入您的监控代码, 使用环境预置的 SDK 可以访问系统数据库, 然后获取您感兴趣的数据并进行监控, 上方的演示动画中获取了所有的交易历史, 更多的功能和用法需要您自己去发掘, 相关详细信息请参考我们的开源代码库
- 调试
  在配置的第二步, 您将可以看到此脚本的运行结果或者程序异常时的错误信息, 依赖此功能可以进行简单的开发调试
...
1. 安装 TermiPass 浏览器插件并 创建/导入 您的助记词
2. 网页通过 TermiPass 插件获取您的 did
3. 上传您的身份认证资料
4. 如实填写 KYC 信息
5. 使用 TermiPass 对您的 KYC 信息进行签名, 并提交到 relay 服务器
6. 联系或等待审核人员对您的 KYC 申请进行审核
7. 审核过后, 通过网页获取您的 VC 凭证并存入 TermiPass, 然后使用 TermiPass 签名提交您的 VP 凭证
...
1. 报价阶段
   ...
   3. 输入您的收款地址
   4. 从报价列表中选择您满意的报价
      - 灰色的报价是不满足 LP 要求的报价, 无法选择, 可能是 KYC 状态, 也可能是您的年龄或国家
      - LP name 后面的星代表 LP 的信用分, 依据最近一段时间的历史交易完成度, 平均交易时间, 被投诉的数量等情况综合计算得出
   5. 使用 钱包(例如:metamask) 确认交易信息并签名

2. 交易阶段

在每一步的时间限制内, 通过钱包发送交易, 如果您发现 LP 锁定的代币或者数量与报价中的信息不符, 或者是 LP 没有在时间限制内操作 token 锁仓, 可以点击 complain 进行投诉, 不论交易因何种原因未成功进行, 您都可以在 refund 倒计时结束后操作退款
