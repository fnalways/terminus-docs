---
outline: [2, 3]
description: 在 Olares 上安装 Steam，配置串流服务，并使用 Moonlight 从本地或远程网络串流 Steam 游戏。
--- 

# 使用 Steam 串流你喜爱的游戏

想要利用 Olares 的强大性能放松一下？没有问题。借助 Steam 应用，Olares 轻松化身为 Steam 串流服务器，让你可以通过 Moonlight 或 Steam Link 客户端在任意兼容设备上畅玩你最爱的游戏大作。

本教程将带你完成 Steam 应用的安装、串流服务的配置，以及通过 Moonlight 客户端进行游戏串流。

## 目标
通过本教程，你将学习：

- 在 Olares 上安装 Steam，并打开 Windows 游戏的兼容性。  
...
1. 打开 Olares 应用市场，在"娱乐"分类下找到 **Steam** 并点击安装。
2. 安装完成后，打开 **Steam** 应用，点击 **Connect** 进入安装后台。 
3. 点击 **Install** 按钮以安装并更新 Steam 客户端。安装完毕后会自动跳转到 Steam 登录页面。
...
::: tip 重试安装
由于国内网络环境限制，Steam 安装和更新可能失败，此时可在 Steam 控制台左上角进入 **Applications** > **Internet** > **Steam** 以重新安装。多次尝试一般可以成功。
:::
...
Steam 应用集成了 Sunshine 串流服务器。要使用 Moonlight 客户端串流，还需在 Moonlight 上将游戏主机与 Sunshine 配对。
...
- 在 Steam 控制台左上角进入 **Applications** > **Settings** > **Display** 中修改分辨率。  
